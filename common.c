...
void handle_path_scroll(int ch, int *offset, const char *dir_path)
{
    ...
    int visual_width = calculate_visual_width(dir_path);
...
void top(WINDOW *win, const char *dir_path, int *visual_offset)
{
    ...
    if (*visual_offset > max_offset)
        *visual_offset = max_offset;
...
// потом посмотреть можно ли перенести в библиотеку?
...
{
    if (!win || !title || !title_left || !title_right) return;
    wchar_t wtitle[INNER_WIDTH + 1] = {0};
    prepare_display_wstring(title, actual_width - 4, wtitle,
                            sizeof(wtitle) / sizeof(wchar_t), 0, L"..", 0, 0);
    size_t title_wlen = wcslen(wtitle);
    int title_width = compute_wchar_width(wtitle, title_wlen);
    int title_len = title_width + 4;
    int title_start = (actual_width - title_len) / 2;
    if (title_start < 1) title_start = 1;
    if (title_start + title_len > actual_width - 1)
        title_start = actual_width - title_len - 1;
    mvwprintw(win, y, title_start, "%s ", title_left);
    mvwaddnwstr(win, y, title_start + 2, wtitle, (int)title_wlen);
...
#define COLOR_PAIR_FATAL        (COLOR_PAIR_RED | A_BOLD)
...
void msg_update_timeouts(void) {
    if (current_message.visible &&
        ...
        if (time(NULL) - current_message.show_time >= 2) {
